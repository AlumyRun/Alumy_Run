    ...
    try {
      ...
      if (user) {
        ...
        const { error } = await supabase.from('workouts').update({
          ...
          planned: updatedWorkout.planned, completed: updatedWorkout.completed, updated_at: new Date()
      ...
      if (user) {
        ...
        if (!error && data) {
          const mappedRaces = data.map(r => ({
            ...
            resultTime: r.result_time || null,
            resultDistance: r.result_distance ? parseFloat(r.result_distance) : null
          }));
    ...
    try {
      ...
      if (user) {
        ...
        const { error } = await supabase.from('races').insert([{
          ...
          result_time: race.resultTime || null,
          result_distance: race.resultDistance || null
        }]);
    ...
    try {
      ...
      if (user) {
        ...
        const { error } = await supabase.from('races').update({
          ...
          status: updatedRace.status || 'planned',
          result_time: updatedRace.resultTime || null,
          result_distance: updatedRace.resultDistance || null,
          updated_at: new Date()
      ...
      for (const r of localRaces) {
        ...
        if (!data || data.length === 0) {
          await supabase.from('races').insert([{
            ...
            status: r.status || 'planned', result_time: r.resultTime || null, result_distance: r.resultDistance || null
          }]);
        }
      }
      await this.markMigrationCompleted(userId);
    } catch (err) { throw new Error('Falha ao sincronizar dados offline.'); }
  }
