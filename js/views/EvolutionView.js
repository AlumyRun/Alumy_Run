...
export class EvolutionView {
  ...
  render(container) {
    ...
    // Leitura estrita dos treinos REALIZADOS (status: 'completed' e objeto completed válido)
    const completedWorkouts = rawWorkouts
      .filter(w => w.status === 'completed' && w.completed && parseFloat(w.completed.distance) > 0)
      .sort((a, b) => new Date(a.date) - new Date(b.date)); // Ordenação cronológica para cálculos
    ...
    // ESTADO VAZIO: Se não houver treinos concluídos no período
    ...
    contentDiv.innerHTML = `
      ...
      <div class="dash-metrics-row" style="margin-bottom: 20px;">
        ...
        <div class="dash-card">
          ...
          <div class="dash-card-subtext">No período selecionado</div>
        </div>
      </div>

      <!-- METRICAS ADICIONAIS + COMPARAÇÃO SEMANAL -->
          ...
          <div style="font-size: 0.85rem; color: var(--text-primary); margin-top: 6px;">
            ${comparison.hasEnoughData ? `
              <div style="display:flex; gap:12px; font-weight:700;">
                ...
                <span style="color: ${comparison.paceDiffSec <= 0 ? '#10b981' : '#ef4444'};">${comparison.paceDiffSec <= 0 ? '' : '+'}${comparison.paceDiffSec}s/km</span>
              </div>
      ...
      <!-- GRÁFICOS (QUILOMETRAGEM POR SEMANA E EVOLUÇÃO DO PACE) -->
    ...
    // Desenha os gráficos utilizando a instância global de Chart.js
  ...
  calculateStats(workouts) {
    ...
    workouts.forEach(w => {
      ...
      // Garantia de cálculo do tempo e conversão para segundos
      const secs = this.parseTimeToSeconds(w.completed.time);
      totalSeconds += secs;

      // Determina ou calcula o pace real
      ...
      // Identificação matemática do Melhor Pace (menor tempo por km)
      if (dist > 0 && secs > 0) {
    ...
    // Formatação do Tempo Total (ex: 10h 42min)
  ...
  calculateWeeklyComparison(allCompletedWorkouts) {
    ...
    const currentWeekWorkouts = allCompletedWorkouts.filter(w => {
    ...
    if (currentWeekWorkouts.length === 0 || prevWeekWorkouts.length === 0) {
    ...
    // Diferença do pace médio em segundos
    const curSecs = currentWeekWorkouts.reduce((acc, w) => acc + this.parseTimeToSeconds(w.completed.time), 0);
    const prevSecs = prevWeekWorkouts.reduce((acc, w) => acc + this.parseTimeToSeconds(w.completed.time), 0);

    const curPaceSec = curKm > 0 ? (curSecs / curKm) : 0;
    const prevPaceSec = prevKm > 0 ? (prevSecs / prevKm) : 0;
    const paceDiffSec = Math.round(curPaceSec - prevPaceSec);
  ...
  renderPaceChart(workouts) {
    ...
    workouts.forEach(w => {
      ...
      if ((!paceStr || paceStr === '0:00') && dist > 0) {
        paceStr = StorageService.calculatePace(dist, w.completed.time);
      }
      ...
      if (paceStr) {
        ...
        if (parts.length === 2) {
          paceDataInMinutes.push(parts[0] + (parts[1] / 60));
        }
      }
    });
    ...
    this.paceChart = new window.Chart(ctx, {
      ...
      options: {
        ...
        scales: {
          y: {
            reverse: true, // Pace menor (mais rápido) fica no topo do gráfico
            grid: { color: '#e2e8f0' },
            ticks: {
              callback: (value) => {
